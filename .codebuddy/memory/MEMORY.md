...
- **原型审查坑（`prototype/`）**：① 综合页容器 `#main-view`、独立页 `#page-body` 必须统一用 `getMain()` 取，否则独立页二次渲染拿到 `null` 崩溃；② `app-data.js` 人物为数据驱动派生（IIFE，`window.APP_DATA`，现状 38 位/6 关系），原型内 `renderStats` 文案与页脚兜底若写死数字会与真实派生值脱节，须动态读 `DATA`；③ 改 `prototype.*` 前务必先 `read_file` 确认当前内容，版本号以 `package.json` 为权威源（HEAD 长期高于会话假设值）。
