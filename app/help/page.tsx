    ...
    <section>
      ...
      <div className="card stat-panel" style={{ lineHeight: 2, fontSize: 15 }}>
        ...
        <ul style={{ paddingLeft: 20, margin: "4px 0" }}>
          ...
          <li><strong>导出书单</strong>：点击「导出书单（含原文直链）」可将当前筛选结果导出为 CSV，含书名、馆藏、子类、大小与原文直链，便于离线整理（仅含索引与原文直链，不含正文）。</li>
        </ul>
      </div>
      ...
      <div className="card stat-panel" style={{ lineHeight: 2, fontSize: 15 }}>
        <p>点击书籍的「阅读」进入阅读页。原文按需从上游数据源（殆知阁）加载，不预取全量数据：</p>
        <ul style={{ paddingLeft: 20, margin: "4px 0" }}>
          <li><strong>分片懒加载</strong>：超大古籍按章节分段逐段拉取，首屏仅加载当前章节，目录可快速跳转。</li>
          <li><strong>本地缓存</strong>：较小书籍整本下载后存入浏览器本地缓存（有容量上限，自动清理最久未读内容），再次打开秒开、无需重下。</li>
      ...
      {/* 五、历史与下载 */}
      <h3 className="section-title" id="history">五、阅读历史、书签与导出</h3>
      <div className="card stat-panel" style={{ lineHeight: 2, fontSize: 15 }}>
        <ul style={{ paddingLeft: 20, margin: "4px 0" }}>
          ...
          <li><strong>书籍收藏</strong>：书名旁的「☆」可将书籍加入书签；顶部导航「书签」页集中管理（打开 / 移除 / 清空），并以角标显示收藏数。</li>
          <li><strong>阅读位置书签</strong>：阅读页点「保存当前位置」记下当前章节与页码，书签面板可一键跳回。</li>
          <li><strong>跨设备迁移</strong>：书签页支持「导出 / 导入 JSON」，可将收藏手动迁移到其他浏览器或设备。</li>
        </ul>
      ...
      <div className="card stat-panel" style={{ lineHeight: 1.95, fontSize: 15 }}>
        ...
        <p>A：标题检索只在书名中匹配，速度快；全文检索会扫描正文。全文检索依赖站点预生成的全文索引，若索引尚未就绪，系统会自动回退为标题检索并提示。</p>
