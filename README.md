...
- **书签与收藏**：书架式「书籍收藏」跨会话保存 + 阅读位置书签（多锚点一键跳回），支持导出 / 导入 JSON 跨设备迁移；全程本地 `localStorage` 存储，零后端。
- **愉悦体验**：克制而文气的微交互——墨韵按钮水光、朱砂点睛卡片、纸墨呼吸光晕、卷首阅读进度、页脚彩蛋，全站动效尊重系统「减弱动态效果」偏好，读屏与键盘操作无障碍友好。
