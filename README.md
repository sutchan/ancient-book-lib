...
- 当前版本：**v1.3.2**（v1.2.5 原型页脚数据统计；v1.2.7 书目页精简；v1.2.8 人物数据派生；v1.2.9 全站 navbar/索引/XSS 修复；v1.2.10 原型审查修复；v1.3.0 去除演示数据、直连殆知阁 v20 全量 15,694 部 txt，检索重构为标题检索 + 可构建的全文倒排索引；v1.3.1 超大书 HTTP Range 分片懒加载、馆藏书单导出、人物/关系数据驱动接线、全文索引与章节清单构建整合；**v1.3.2 原型致命回归修复（getMain 无限递归）与模块拆分**）
