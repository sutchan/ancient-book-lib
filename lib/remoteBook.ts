...
export async function fetchChapterText(
  ...
  // fetchWithTimeout 内部会补 "bytes=" 前缀。chapters.json 的 end 为半开区间上界（不含），
  // 而 HTTP Range 的 end 为闭区间，故需减 1，否则会多请求下一章首字节（多字节字符末端乱码）。
  const range = `${ch.start}-${Math.max(ch.start, ch.end - 1)}`;
